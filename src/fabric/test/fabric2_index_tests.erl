...
single_update(#{db1 := Db}) ->
    ...
    meck:wait(Mod, build_indices, ['_', '_'], 2000),
    ?assertEqual(1, meck:num_calls(Mod, build_indices, ['_', [{'_'}]])).
...
multiple_updates(#{db1 := Db}) ->
    ...
    timer:sleep(500),
    ?assert(meck:num_calls(Mod, build_indices, ['_', '_']) =< 3).
...
skip_db_if_no_ddocs(#{db2 := Db}) ->
    ...
    timer:sleep(500),
    ?assertEqual(0, meck:num_calls(Mod, build_indices, '_')).
